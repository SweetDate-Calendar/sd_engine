defmodule CLPWeb.CalendarLiveTest do
  use CLPWeb.ConnCase
  ...
  describe "Index" do
    ...
    test "saves new calendar", %{conn: conn} do
      ...
      assert {:ok, form_live, _} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"/calendars/new")
    ...
    test "updates calendar in listing", %{conn: conn, calendar: calendar} do
      ...
      assert {:ok, form_live, _html} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"/calendars/#{calendar}/edit")
  ...
  describe "Show" do
    ...
    test "updates calendar and returns to show", %{conn: conn, calendar: calendar} do
      ...
      assert {:ok, form_live, _} =
               ...
               |> element("a", "Edit")
               |> render_click()
               |> follow_redirect(conn, ~p"/calendars/#{calendar}/edit?return_to=show")
      ...
      assert {:ok, show_live, _html} =
               ...
               |> form("#calendar-form", calendar: @update_attrs)
               |> render_submit()
               |> follow_redirect(conn, ~p"/calendars/#{calendar}")
