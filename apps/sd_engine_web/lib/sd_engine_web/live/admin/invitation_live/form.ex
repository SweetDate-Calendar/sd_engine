defmodule SDWeb.Admin.InvitationLive.Form do
  ...
  def handle_event("validate", %{"invitation" => invitation_params}, socket) do
    changeset = Notifications.change_invitation(socket.assigns.invitation, invitation_params)
  ...
  defp save_invitation(socket, :edit, invitation_params) do
    case Notifications.update_invitation(socket.assigns.invitation, invitation_params) do
      {:ok, invitation} ->
        {:noreply,
         socket
  ...
  defp save_invitation(socket, :new, invitation_params) do
    case Notifications.create_invitation(invitation_params) do
      {:ok, invitation} ->
        {:noreply,
         socket
  ...
  defp return_path("show", event_id, invitation) do
    ~p"/events/#{event_id}/event_invitations/#{invitation}"
  end
end
